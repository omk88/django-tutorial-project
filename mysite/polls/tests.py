...
class Part3Marker(Marker):
    ...
    def test_detail_view(self, cls):
        ...
        try:
            response = cls.client.get(reverse('detail', kwargs={'question_id':question.id}), follow=True)
        except:
            response = cls.client.get(reverse('polls:detail', kwargs={'question_id':question.id}), follow=True)            
        self.assertEquals(response.status_code, 200)
        cls.report['marks'] += 0.5
